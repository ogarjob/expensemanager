<div x-data="{ open: false }"
    ...
    <button @click.prevent="open = ! open"
        ...
        <div class="flex">
            <x-livewire-powergrid::icons.download class="h-6 w-6 text-slate-500"/>
        </div>
    </button>
    ...
    <div x-show="open"
        ...
        @if(in_array('excel', data_get($setUp, 'exportable.type')))
            <div class="flex px-4 py-2 text-slate-400">
                <span class="w-12">@lang('Excel')</span>
                <a x-on:click="$wire.call('exportToXLS'); open = false"
                   href="#"
                   class="px-2 block text-slate-800 hover:bg-slate-50 hover:text-black-300 rounded">
