<div class="items-center justify-between sm:flex">
    <div class="items-center justify-between w-full sm:flex-1 sm:flex">
        @if($recordCount === 'full')
            <div>
                <div class="mr-2 leading-5 text-center text-slate-700 text-md sm:text-right">
        ...
        @elseif($recordCount === 'short')
            <div>
                <p class="mr-2 leading-5 text-center text-slate-700 text-md">
                    <span class="font-semibold firstItem"> {{ $paginator->firstItem() }}</span>
                    -
                    <span class="font-semibold lastItem"> {{ $paginator->lastItem() }}</span>
                    |
            ...
            <nav role="navigation" aria-label="Pagination Navigation" class="items-center justify-between sm:flex">
                <div class="flex justify-center mt-2 md:flex-none md:justify-end sm:mt-0">
                    ...
                    @if(!$paginator->onFirstPage())
                        <a
                            class="px-2 py-1 pt-2 m-1 text-center text-white bg-slate-500 border-slate-400 rounded cursor-pointer border-1 hover:bg-slate-600 hover:border-slate-800"
                            wire:click="gotoPage(1)"
                        >
                            <x-livewire-powergrid::icons.chevron-double-left/>
                        </a>

                        <a class="px-2 py-1 pt-2 m-1 text-center text-white bg-slate-500 border-slate-400 rounded cursor-pointer border-1 hover:bg-slate-600 hover:border-slate-800"
                        ...
                        @if (is_array($element))
                            ...
                            @foreach ($element as $page => $url)
                                @if ($paginator->currentPage() > 3 && $page === 2)
                                    <div class="mx-1 mt-1 text-slate-800">
                                ...
                                @elseif ($page === $paginator->currentPage() + 1 || $page === $paginator->currentPage() + 2 || $page === $paginator->currentPage() - 1 || $page === $paginator->currentPage() - 2)
                                    <a class="px-2 py-1 m-1 text-center text-white bg-slate-500 border-slate-400 rounded cursor-pointer border-1 hover:bg-slate-600 hover:border-slate-800"

                                       wire:click="gotoPage({{$page}})">{{ $page }}</a>
                                @endif

                                @if ($paginator->currentPage() < $paginator->lastPage() - 2  && $page === $paginator->lastPage() - 1)
                                    <div class="mx-1 mt-1 text-slate-600">
                                        <span>.</span>
                                        <span>.</span>
                                        <span>.</span>
                                    </div>
            ...
            @if ($paginator->hasPages() && $recordCount == 'min')
                <nav role="navigation" aria-label="Pagination Navigation" class="items-center justify-between sm:flex">
                    ...
                    <span>
                        ...
                        @if ($paginator->onFirstPage())
                            <button disabled
                                    class="p-2 m-1 text-center text-slate-400 bg-slate-200 border-slate-400 rounded border-1">
                                <x-livewire-powergrid::icons.chevron-double-left/>
                            </button>
                        @else
                            @if(method_exists($paginator,'getCursorName'))
                                <button
                                    ...
                                    class="p-2 m-1 text-center text-white bg-slate-500 border-slate-400 rounded cursor-pointer border-1 hover:bg-slate-600 hover:border-slate-800">
                                        <x-livewire-powergrid::icons.chevron-double-left/>
                                </button>
                            @else
