...
<div x-cloak
    ...
    <div @class([
        ...
        @if(!$inline)
            <label class="text-gray-700">{{ data_get($multiSelect, 'label') }}</label>
        @endif
        ...
        <div class="{{ $theme->baseClass }}" style="{{ $theme->baseStyle }}">
            <div class="flex flex-col items-center relative">
                <div x-on:click="show = true" class="w-full"
                    ...
                    <div
                        class="my-1 border focus:bg-white focus:border-slate-500 border-slate-300 rounded flex">
                        <div class="flex flex-auto flex-wrap">
                            <div x-cloak
                                 x-show="selected.length > 0"
                                 x-text="selected.length"
                                 class="text-sm text-slate-500 flex p-1 items-center justify-center"></div>
                            <template x-for="(option, index) in selected" :key="index">
                                <div
                                    class="flex justify-center items-center m-1 cursor-pointer inline-flex rounded-full items-center py-0.5 pl-2.5 pr-1 text-sm font-medium bg-slate-200 text-slate-700">
                                    <div
                                        class="pl-0 align-middle cursor-pointer hover:text-black hover:text-current text-left text-xs font-medium text-slate-700 tracking-wider"
                                        x-text="option.text"></div>

                                    <button type="button"
                                            x-on:click="remove(option.value)"
                                            class="flex-shrink-0 ml-0.5 h-4 w-4 rounded-full inline-flex items-center justify-center text-slate-600 hover:bg-slate-200 hover:text-slate-500 focus:outline-none focus:bg-slate-500 focus:text-slate-200">
                            ...
                            <div x-show="selected.length === 0" class="flex-1">
                                <input readonly
                                       ...
                                       class="w-full block bg-white text-slate-700 py-2 text-sm px-3 leading-tight focus:outline-none"
                        ...
                        <div
                            ...
                            <button type="button" x-show="show" x-on:click="show = true"
                                ...
                                <x-livewire-powergrid::icons.up
                                    class="w-4 h-4 text-slate-400 cursor-pointer"/>
                            </button>
                ...
                <div class="w-full px-4">
                    <div x-show="show"
                         ...
                         class="absolute rounded-lg overflow-hidden z-100 shadow-md top-100 border border-slate-300
                                bg-white z-50 w-full left-0 rounded max-h-select overflow-y-auto
                               "
                                ...
                                <div>
                                    ...
                                    <div
                                        ...
                                        class="relative group cursor-pointer hover:bg-slate-50 w-full border-slate-100 rounded-t border-b
                                              ">
                                        <div x-bind:class="option.selected ? 'border-slate-600' : ''"
                                             class="flex w-full items-center p-2 bg-slate-200 border-transparent relative">
                                            <div class="w-full items-center flex">
                                                <div
